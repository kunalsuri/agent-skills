...
- 💾 **Durable** — the flagship skill, [`resumable-sdd`](#-skills), commits the
  *plan and the progress* to git, so an interrupted task resumes from two files
...
## 🧭 Where `resumable-sdd` fits

It's deliberately the **light end** of the SDD spectrum — for small-to-medium
tasks where the full ceremony is overkill but losing an hour of uncommitted work
would still hurt.

```
  heavier ┃  GitHub Spec Kit      full 7-phase pipeline, 30+ agents, skills mode
          ┃  AI-Fication-Kit      my own full-SDD project + dev loop
  lighter ┃  resumable-sdd  ◀──    this repo: one spec, one checklist, commit-per-step
```
...
## 🗂️ Part of a bigger effort — SkillDeck

This repo is my *hand-crafted* skills. It feeds into a broader project:

### [**SkillDeck**](https://github.com/kunalsuri/SkillDeck) — a cross-company directory & knowledge base for AI agent skills

> A modern directory and knowledge base designed to **catalog, cluster, and
> verify** AI agent skills, workflows, and tools.

Where this repo is one author's skills, **SkillDeck aggregates skills and data
from multiple companies** — ingesting from official sources like **Anthropic,
Google, and Vercel** and organizing them by capability (Documents, Cloud Ops, Data
Analysis, Frontend, Testing, …) so any agent framework can discover, read, and run
them.

- 🏗️ **Three-tier architecture** — an Astro static frontend on Vercel, a local
  Python "kitchen" pipeline (ingest → dedupe → cluster → curate), and per-source
  JSON data versioned on GitHub.
- 🧪 **Verified, not just scraped** — MinHash deduplication + agent-driven
  clustering produce explainer cards, then a **human-in-the-loop** review portal
  promotes or rejects each skill for safety, accuracy, and utility.
- 🔁 **Efficient sync** — ETag-based GitHub caching and atomic writes keep the
  catalog fresh without redundant fetches.
